...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("ROXYMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("ROXYMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("ROXYMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("roxy-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("ROXYMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running ROXYMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadROXYMiner() exception");
    } catch (...) {
        LogPrintf("ThreadROXYMiner() exception");
    }

    LogPrintf("ThreadROXYMiner exiting\n");
}
